 ...
 const saveSchema = async () => {
  try {
    ...
    Swal.fire({
      icon: "success",
      title: "Saved!",
      text: "Schema has been successfully updated.",
    ...
    Swal.fire({
      icon: "error",
      title: "Error!",
      text: err.message || "Something went wrong while saving schema.",
...
const removeField = (sIdx, fIdx) => {
  ...
  const addSection = () => {
    setSchema((prev) => {
      const next = { ...prev };
      next.sections.push({ title: `Section ${next.sections.length + 1}`, fields: [] });
      persistSchema(next);
      return next;
    });
  };

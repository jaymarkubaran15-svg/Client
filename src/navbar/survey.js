...
const [selectedBatch, setSelectedBatch] = useState("");
  ...
  useEffect(() => {
    ...
    const getAllBatches = () => {
  const batches = submissions.map((s) => s["Please select year of graduation"] || "No Year");
  return [...new Set(batches)];
};

  // Get unique programs dynamically
  const getAllPrograms = () => {
    const programs = submissions.map((s) => s["Program Completed"] || "Unknown Program");
  ...
  return (
    (!selectedProgram || s["Program Completed"] === selectedProgram) &&
    (!selectedBatch || s["Please select year of graduation"] === selectedBatch)
  );
});
    ...
    filteredFields.forEach((field, idx) => {
      ...
      if (chartData.length) {
        const tableData = chartData.map((d) => [
          d.name,
          d.value,
          `${((d.value / filteredSubmissions.length) * 100).toFixed(1)}%`,
        ]);
        autoTable(doc, {
          startY,
      ...
      if (!selectedProgram) {
        ...
        programs.forEach((prog) => {
          const progSubmissions = submissions.filter(
            (s) => (s["Program Completed"] || "Unknown Program") === prog
          );

...
const exportPDF = async () => {
  ...
  for (let i = 0; i < filteredUsers.length; i++) {
    ...
    const userDetails = [
      ...
      ["Email", user.email || "-"],
      ["Region Of Origin", user.regionOfOrigin || "-"],
      ["Province", user.province || "-"],
      ["Location Residence", user.locationResidence || "-"],
    ...
    try {
      ...
      if (answers && Object.keys(answers).length > 0) {
        doc.setFontSize(14);
        doc.text("GTS ANSWERS", 14, currentY);
        currentY += 10;

        let questionCounter = 1;

        const renderAnswers = (obj, prefix = "") => {
          let body = [];
          ...
          Object.entries(obj).forEach(([key, value]) => {
            if (
              ...
              const nested = renderAnswers(value, `${prefix}${key}.`);
              body.push(...nested);
        ...
        const body = renderAnswers(answers);
      ...
      console.error(`Error fetching survey answers for ${user.first_name}:`, error);
    }
  }

  doc.save("alumni_bulk_export.pdf");
};
                 ...
                 <tbody>
                    {filteredUsers.length > 0 ? (
                        ...
                        <tr key={index} className="border-b">
                            ...
                            <td className="p-3">{user.work_title || "-"}</td>
